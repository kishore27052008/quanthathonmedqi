...
export default function Patients() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* Search Input */}
      <div className="bg-[#0F172A] p-4 rounded-2xl border border-slate-800/80 shadow-sm flex items-center gap-3">
              ...
              <tbody className="divide-y divide-slate-800 text-xs font-medium">
                {filteredPatients.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="py-4 px-6 font-mono font-bold text-white">{p.mrn || `P-${p.id}`}</td>
                    <td className="py-4 px-6 font-bold text-white">{p.full_name}</td>
                    <td className="py-4 px-6 text-slate-300">
                      {p.date_of_birth} ({p.gender})
                    </td>
                    <td className="py-4 px-6 text-slate-300 flex items-center gap-1.5 mt-2">
                      <Phone className="w-3.5 h-3.5 text-slate-500" /> {p.contact_number}
                    </td>
                    <td className="py-4 px-6 text-slate-300">
                      <span className="inline-flex items-center gap-1">
                        <Building2 className="w-3.5 h-3.5 text-slate-500" /> {p.hospital_id}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-right">
                      <button
                        onClick={() => handlePredictForPatient(p)}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-brand-950/80 hover:bg-brand-900 text-brand-300 text-xs font-bold border border-brand-800 transition-all"
                      >
                        <Stethoscope className="w-3.5 h-3.5 text-brand-400" /> Predict Risk
                      </button>
                    </td>
                  </tr>
                ))}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#0F172A] rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-800 space-y-4">
            ...
            <form onSubmit={handleRegisterPatient} className="space-y-3 text-xs">
              <div>
                <label className="block font-bold text-slate-300 mb-1">MRN Code</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-xl font-medium text-white"
                  >
              ...
              <div>
                <label className="block font-bold text-slate-300 mb-1">Hospital / Clinic ID</label>
                <input
                  type="text"
              ...
              <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-800">
                <button
                  ...
                  className="px-4 py-2 border border-slate-800 rounded-xl font-semibold text-slate-300 hover:bg-slate-800"
                >
                  Cancel
                </button>
                <button
